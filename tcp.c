  ...
  switch (pcb->state) {
    case TCP_PCB_STATE_LISTEN:
      ...
      if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
        ...
        // also ignoring precedence check
        pcb->local = *local;
        pcb->foreign = *foreign;
        ...
        // ignoring: note that any other incoming control ot data combined with
        // syn will be processed in the syn-recieved state. but processing of
...
int tcp_open_rfc793(struct tcp_endpoint *local, struct tcp_endpoint *foreign,
  ...
  if (active) {
    errorf("active open is not implemented, sorry");
    tcp_pcb_release(pcb);
    pthread_mutex_unlock(&mutex);
    return -1;
  } else {
    debugf("passice open: local=%s:%u, waiting for connection...",
