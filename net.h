...
#define NET_IFACE_FAMILY_IP_V6 2
...
struct net_device {
  ...
  uint8_t addr[NET_DEVICE_ADDR_LEN];  // mac addressF
  union {
  ...
  void
      *priv;  // private, デバイスドライバが使うプライベートなデータへのポインタ
};
...
struct net_device_ops {
  ...
  int (*transmit)(struct net_device *dev, uint16_t type, const uint8_t *data,
                  size_t len, const void *dst);  // これは必須
...
struct net_iface{
  ...
  int family; // depends on implementation of protocols
};
